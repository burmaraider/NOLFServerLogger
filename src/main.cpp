...
DWORD WINAPI Init(HMODULE hModule)
{
	...
	//Hook the main textbox in NolfServ.exe
	oAddMessageToBox = (tSub_404CC0)TrampHook((char*)p_addrServerApp + 0x4CC0, (char*)&StuffMessageIntoLog, 6);
	if (oAddMessageToBox != nullptr)
	{
		//myLog.SetLogFile();
	}

	oTest = (tSub_06D80)TrampHook((char*)p_addrServerApp + 0x06D80, (char*)&Test, 6);

	while (true)
	{
		//Find object.lto and hook
		p_addrObjectLto = FindObjectAddress("object.lto");
		if (p_addrObjectLto != 0x0)
		{
			//Hook OnClientEnterWorld function
			oOnClientEnterWorld = (tSub_D6DC0)TrampHook((char*)p_addrObjectLto + 0xD6DC0, (char*)&OnClientEnterWorld, 9);

			//Hook OnClientLeaveWorld
			oOnClientExitWorld = (tSub_D72A0)TrampHook((char*)p_addrObjectLto + 0xD72A0, (char*)*OnClientExitWorld, 6);

			return false; //finished all of our hooks
		}
		Sleep(2000); //wait 2 seconds before attemping to find object.lto again...
	}
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved) {
	...
	case DLL_PROCESS_ATTACH:
	{
		...
		if (winmm.dll == false)
		{
			MessageBox(0, "Cannot load original winmm.dll library", "Proxy", MB_ICONERROR);
			ExitProcess(0);
		}
		...
		if (myLog.GetExeFileNameNoPath() == "NolfServ.exe")
		{
			sig.GetProcess("NolfServ.exe");
			p_addrServerApp = FindObjectAddress("NolfServ.exe");
			HANDLE hHandle = CreateThread(nullptr, 0, (LPTHREAD_START_ROUTINE)Init, hModule, 0, nullptr);
			if(hHandle >0 )
				CloseHandle(hHandle);
		}
