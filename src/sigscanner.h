...
public:
	...
	std::string ReadStringAtMemory(DWORD Address, int length)
	{
		char* szTemp = new char[length];
		ReadProcessMemory(TargetProcess, (LPCVOID)Address, &szTemp, length, NULL);
		std::string wat(szTemp);
		delete[] szTemp;
		return wat;
	}

	char* ReadText(DWORD Address, int address) {
		if (address == -1)
			return (char*)"-1";
		char buffer = !0;
		...
		int i = 0;
		while (buffer != 0) {
			BOOL success = ReadProcessMemory(TargetProcess, (LPCVOID)Address, &buffer, NumberOfBytesToRead, &NumberOfBytesActuallyRead);
